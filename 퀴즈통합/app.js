...
function singleFn() {
	...
	previewBtn.off().on('click', function () {
		...
		answer.eq(ans[pageNum][0] - 1).toggleClass('correct');
	});
...
function essayFn() {
	let content = contents.eq(pageNum),
		ansArea = content.find($('.input'));
		// submitbtn = content.find($('.submitBtn')),
		// previewBtn = content.find($('.preview'));
...
function drawingFn() {
	...
	let canvasAns = content.find($("#canvas_answer"));
	let canDiv = content.find($(".answer"));
	let ctx = canvas[0].getContext("2d");
	let ctxAns = canvasAns[0].getContext("2d");

	let dragObj = content.find($('.dragObj')),
		dropObj = content.find($('.dropObj')),
		objCon = content.find($('.objContainer')),
		radius,
		startX = [],
		startY = [],
		originX = [],
